...
def compute_fiq_val_metrics(relative_val_dataset: FashionIQDataset, blip_model, index_features: torch.tensor,
                            index_names: List[str], txt_processors, save_memory=True) -> Tuple[float, float]:
    """
...
def main():
    ...
    parser.add_argument("--eval-mode", default="dynamic", type=str, help="Evaluation mode: 'dynamic' or 'fusion'")
    ...
    if args.dataset == 'CIRR':
        blip_validate_cirr(args.blip_model_name, args.blip_model_path, args.transform, args.target_ratio, args.eval_mode)




def blip_validate_cirr(blip_model_name, blip_model_path, transform, target_ratio, eval_mode):
